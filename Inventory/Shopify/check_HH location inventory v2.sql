...
where v.in_store = '1' --and sv.environment <> 'production' and sm.id = 'gid://shopify/Location/79232434312'
...
drop table if exists #total
 ...
 --select * from #format where sku = 'DF7004WE'
 ...
 shop_open_orders,
 stock_pit,
 floor(IIF(in_quantity_on_hand - in_committed_quantity - st1ProductReserve - in_sfty_stk - amazon_pending + shop_open_orders + stock_pit < 0, 0, in_quantity_on_hand - in_committed_quantity - st1ProductReserve - in_sfty_stk - amazon_pending + shop_open_orders + stock_pit))  as total_product_inventory
 ...
 select
 k.sku,
 min(floor(IIF((f.in_quantity_on_hand - f.in_committed_quantity - f.st1ProductReserve - f.in_sfty_stk - f.amazon_pending + f.shop_open_orders + f.stock_pit)/ k.component_quantity < 0, 0, (f.in_quantity_on_hand - f.in_committed_quantity - f.st1ProductReserve - f.in_sfty_stk - f.amazon_pending + f.shop_open_orders + f.stock_pit)/ k.component_quantity)))  as total_kit_inventory
 into #kits
...
join INV.ShopifyLocationMap sm
on sm.parentid = sv.id
where sm.id = 'gid://shopify/Location/79232434312' --and j.sku = '1931549'
order by sku
